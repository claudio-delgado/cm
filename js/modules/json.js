...
export class JSONFile {
  ...
  build_file = () => {
    const encrypted_data = (json_element) => {
        let data_elem = JSON.parse(JSON.stringify(json_element))
        if(typeof json_element === "object"){
            if(Array.isArray(json_element)){
              json_element.forEach((value, key) => {
                if(typeof json_element[key] === "string" || typeof json_element[key] === "number"){
                    data_elem[this.e(key.toString(), this.current_date)] = this.e(json_element[key], this.e(key.toString(), this.current_date))
                } else {
                    data_elem[this.e(key.toString(), this.current_date)] = encrypted_data(json_element[key])
                }
                delete data_elem[key]
              })
            } else {
              Object.keys(json_element).forEach(key => {
                if(key === "attributes") {debugger}

                if(json_element[key] === null || typeof json_element[key] === "string" || typeof json_element[key] === "number"){
                  let is_date_value = (key === "date")
                  let value = json_element[key] != null ? json_element[key] : "-"
                  if(is_date_value){
                    data_elem[this.e(key, this.current_date)] = this.current_date
                  } else {
                    data_elem[this.e(key, this.current_date)] = this.e(value, this.e(key, this.current_date))
                  }
                } else {
                    data_elem[this.e(key, this.current_date)] = encrypted_data(json_element[key])
                }
                delete data_elem[key]
              })
            }
    ...
    progress.build_data()
    let progress_data = progress.get_data()
    let json_result = {}
    this.json = {}
    this.json = encrypted_data(progress_data)
/*
    ...
    citizens.forEach((citizen) => {
      ...
      encrypted_citizen[this.e("weeksAlive", this.current_date)] = this.e(citizen.weeksAlive.toString(), this.e("weeksAlive", this.current_date))
