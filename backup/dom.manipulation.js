...
class panel{
    ...
    buildPanel(){
        ...
        let paragraphBg, divTitle, divTitleParagraphSpan
        
        //Build title div.
        //--Build div, add paragraph
        d1 = new element("div", "border border-gray-300 border-gray-800 bg-gray-900 text-gray-200 text-xs", [], parentDiv, this.objectName+(this.objectId ? "-"+this.objectId : "")+"-"+this.panelName+"-title"); d1.create()
        p = new element("p", "flex justify-between items-center p-1 ps-2 text-xs "+paragraphBg, [], d1.getNode()); p.create()
        ...
        d = new element("div", "border border-gray-300 dark:border-gray-600 dark:bg-gray-600 text-xs", [], parentDiv, this.objectName+(this.objectId ? "-"+this.objectId : "")+"-"+this.panelName); d.create()
        p = new element("p", "flex w-100 justify-between items-center flex-wrap p-1 text-gray-500 dark:text-gray-300", [], d.getNode()); p.create()

        /*let divSpecificParagraph = p.getNode()
        let divSpecific = d.getNode()*/
        ...
        if(this.panelName == "assignRole"){
            ...
            let buttonText = "", generalRole = "", buttonColours = "", isDisabled = false
            roleIcons.forEach((value, index) => {
                ...
                divSpecificParagraphButton[index] = new element("button", generalRole+" assignableRole text-xs capitalize "+(index+1 < roleIcons.length ? "grow " : "")+"p-2 me-1 mb-1 button border "+buttonColours, [{"key":"data-icon", "value":value.icon},{"key":"data-rolekey", "value":value.key},{"key":"disabled", "value":isDisabled}], p.getNode())
                divSpecificParagraphButton[index].create()
                i = new element("i", "fa fa-"+value.icon+" me-2", [], divSpecificParagraphButton[index].getNode()); i.create()
                divSpecificParagraphButton[index].appendHTML(buttonText)
            })
        }
        if(this.panelName == "newExpedition"){
            //Offer expedition type options
            s = document.createElement("span"); s.classList = ""; s.innerText = translate(language, "Type")+":"
            divSpecificParagraph.appendChild(s)
            s = document.createElement("span"); s.classList = "expeditionType grow flex";
            divSpecificParagraph.appendChild(s)
            b = document.createElement("button"); b.setAttribute("data-type", "of mounts"); b.classList = "text-xs text-white ms-2 grow p-1 button border border-gray-400 bg-green-800"; b.innerText = translate(language, "of mounts")
            s.appendChild(b)
            b = document.createElement("button"); b.setAttribute("data-type", "of ruins"); b.classList = "text-xs text-white ms-2 grow p-1 button border border-gray-400 bg-blue-800"; b.innerText = translate(language, "of ruins")
            s.appendChild(b)
            b = document.createElement("button"); b.setAttribute("data-type", "of combat"); b.classList = "text-xs text-white ms-2 grow p-1 button border border-gray-400 bg-red-800"; b.innerText = translate(language, "of combat")
            s.appendChild(b)
        }
        /*divSpecific.appendChild(divSpecificParagraph)
        parentDiv.appendChild(divSpecific)*/
        ...
        if(this.panelName == "newExpedition"){
            document.querySelectorAll(".expeditionType button").forEach((value) => {
                value.addEventListener("click", (e) => {
                    ...
                    s1 = document.createElement("span"); s1.innerText = translate(language, typeText); s1.classList = "ms-2 p-1 px-2 text-white border border-gray-400 bg-"+background+"-800"
                    s.appendChild(s1)
                    //Show expeditioners assigned
                    //Show expeditioners available
                })
            })
        }
...
let accordionCitizens = (amount = 10) => {
    let citizenBuilder = (index) => {
        ...
        s.appendHTML(": ")
        s1 = new element("span", "font-bold", [{"key":"data-i18n", "value":""}], s.getNode(), "citizen-"+index+"-status"); s1.create(); s1.appendContent("Idle")
        s.appendHTML(".")
        //Second line
        p = new element("p", "mx-1 flex justify-between mb-1 text-xs text-gray-500 dark:text-gray-200", [], d2.getNode()); p.create()
        s = new element("span", "", [], p.getNode()); s.create()
...
let addAssignedWorkerToMount = (citizenIndex, mountClass) => {
    ...
    document.querySelectorAll("#citizen-"+citizenIndex+"-status").forEach((status) => {
        status.innerText = translate(language, "Working")
    })
...
//For certain mount discovered: Remove (Deassign) available worker
